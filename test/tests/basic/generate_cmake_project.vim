...
function! Test_CMake_generate_Validate_create_link_method()
    ...
    call assert_true( filereadable('cmake-build-Release/CMakeCache.txt'), 'CMakeCache.txt should be generated' )
    call assert_false( filereadable('compile_commands.json'), 'compile_commands.json should not be generated' )
...
function! Test_CMake_generate_Generate_cmake_project_in_custom_build_folder()
    call assert_false( isdirectory( 'build_test' ), 'Build directory should not exist' )
...
function! Test_CMake_generate_Generate_cmake_project_in_prefix_build_folder()
    call assert_false( isdirectory( 'cmake-vador-Release' ), 'Build directory should not exist' )
...
function! Test_CMake_generate_Generate_RelWithDebInfo_cmake_project()
    call assert_false( isdirectory( 'cmake-build-RelWithDebInfo' ), 'Build directory should not exist' )
...
function! Test_CMake_generate_Generate_MinSizeRel_cmake_project()
    call assert_false( isdirectory( 'cmake-build-MinSizeRel' ), 'Build directory should not exist' )
...
function! Test_CMake_generate_Emulate_uninitialized_project_with_existing_build_folder()
    call assert_false( isdirectory( 'cmake-build-Release' ), 'Build directory should not exist' )
    silent CMake
...
function! Test_CMake_generate_Call_prepare_file_API_after_project_generation()
    ...
    if isdirectory( 'cmake-build-Release/.cmake/api/v1/reply' )
        ...
        call assert_false( isdirectory( 'cmake-build-Release/.cmake/api/v1/reply' ), 'Reply directory should not exist' )
    endif
endfunction
...
function! Test_CMake_generate_Create_incorrect_folder()
    ...
    call assert_false( isdirectory('custom-folder'), 'Build directory should not exist' )
    call utils#fs#makeDir('custom-folder')
    call assert_false( isdirectory('custom-folder'), 'Build directory should not exist' )
    redir => l:error_message
    1messages
    redir END
    call assert_equal( "\nCannot create a build directory: custom-folder", l:error_message )
endfunction
